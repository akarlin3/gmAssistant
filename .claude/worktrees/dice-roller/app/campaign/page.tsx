    ...
    <main className="min-h-screen p-5 md:p-8">
      <div className="max-w-3xl mx-auto">
        <div className="bg-parchment-soft border border-rule rounded-lg shadow-page p-5 md:p-8 space-y-4">
          <header className="pb-4 border-b border-rule flex items-center justify-between">
            <div>
              <div className="text-xs text-brass-deep font-display uppercase tracking-[0.3em]">
                Campaign Prep
              </div>
              <h1 className="font-display text-3xl text-crimson tracking-wide mt-1">Your Campaigns</h1>
              <p className="text-sm text-ink-soft italic font-serif mt-1">{user.email}</p>
            </div>
            <button onClick={handleSignOut} className="text-xs px-3 py-1 rounded border border-rule text-ink-soft hover:bg-parchment-deep font-display uppercase tracking-wider flex items-center gap-1.5">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {campaigns.map((c) => (
                <Link key={c.id} href={`/campaign/${c.id}`} className="block p-4 rounded border border-rule bg-parchment hover:bg-parchment-deep/50 hover:border-crimson/60 transition-colors shadow-card group">
                  <div className="flex items-center gap-3">
                    <ScrollText size={16} className="text-crimson flex-shrink-0" />
                    <span className="font-display tracking-wide text-ink flex-1 group-hover:text-crimson transition-colors">{c.name}</span>
                    <span className="text-xs text-brass-deep flex items-center gap-1 font-serif italic">
                      <Calendar size={11} />
